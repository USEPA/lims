...
namespace QuantStudio_qPCR
{
    ...
    {
        public override string id { get => "quantstudio_qpcr"; }
        public override string name { get => "QuantStudio_qPCR"; }
        public override string description { get => "Processor used for QuantStudio qPCR translation to universal template"; }
        ...
        public override DataTableResponseMessage Execute()
        {
            ...
            try
            {
                ...
                string tmpDateTime = worksheet.Rows[35][ColumnIndex0.B].ToString().Trim();
                ...
                //If the aliquot name contains 'Pos Ctrl' then all those corresponding UserDefined1 values (column AA)
                ...
                DataRow dr = null;
                for (int row = 47; row < numRows; row++)
                {
                    current_row = row;
                    ...
                    {                        
                        double tmpdPosCtrl;
                        string colAA = worksheet.Rows[row][ColumnIndex0.AA].ToString().Trim();
                        if (!Double.TryParse(colAA, out tmpdPosCtrl))
                            throw new Exception("Unable to parse User Defined1 value for column AA: " + aliquot);
                    ...
                    userDefined1 = worksheet.Rows[row][ColumnIndex0.AA].ToString().Trim();
                ...
                dr["User Defined 1"] = (dPosCtrl / numPosCtrl).ToString();
                dt.Rows.Add(dr);
